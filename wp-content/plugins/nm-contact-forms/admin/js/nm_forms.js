...
jQuery(document).ready(function(){
	...
	jQuery(document).on( 'click', '#toggle_settings',function(event){
	
		event.preventDefault();
		
		if(jQuery(this).hasClass('active')){
			jQuery(this).removeClass('active');
			jQuery('.nm_form_settings').slideUp(500);
		}else{
			jQuery(this).addClass('active');
			jQuery('.nm_form_settings').slideDown(500);
		}
	
	});
	
	jQuery(document).on( 'click', '#nm_save_forms, #nm_top_save_forms',function(event){
	...
	jQuery(document).on( 'click', '#add_form',function(event){
		...
		+'<h3>'+nm_form_title+'<span class="nm_form_delete">x</span><span class="nm_form_add">Add field</span></h3>'
		+'</div>'
