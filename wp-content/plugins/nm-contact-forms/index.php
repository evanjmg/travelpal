...
class nm_forms {
	...
	<h3 class="nm_donate_heading">You will have to find a way to donate</h3>
	Apparently my PayPal account have been suspended due to using "donate" label in the button rather "buy me coffee" or something similar. Turns out only organizations can use donate button. Anyway if you know any way how you could support me, feel free to email at voouss@gmail.com.
	...
	function nm_send($nm_form){
		...
		if(!empty($nm_form['subject'])) $subject = $nm_form['subject'];
		...
		foreach($nm_form['fields'] as $field){
			...
			switch($field['type']){
				case 'text':
					...
					if($nm_form['show_labels']) $form.= '<div class="nm_label">'.$field['title'].'</div>';
